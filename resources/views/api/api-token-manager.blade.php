    ...
    <x-form-section submit="createApiToken">
        ...
        <x-slot name="form">
            ...
            <div class="col-span-6 sm:col-span-4">
                ...
                <x-inputs.default id="name" type="text" class="mt-1 block w-full" wire:model.defer="createApiTokenForm.name" autofocus />
                <x-errors.default for="name" class="mt-2" />
            </div>
                    ...
                    <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach (Laravel\Jetstream\Jetstream::$permissions as $permission)
                            <label class="flex items-center">
                                <x-inputs.checkbox wire:model.defer="createApiTokenForm.permissions" :value="$permission"/>
    ...
    <x-dialog-modal wire:model="managingApiTokenPermissions">
        ...
        <x-slot name="content">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach (Laravel\Jetstream\Jetstream::$permissions as $permission)
                    <label class="flex items-center">
                        <x-inputs.checkbox wire:model.defer="updateApiTokenForm.permissions" :value="$permission"/>
                        <span class="ml-2 text-sm text-gray-600">{{ $permission }}</span>
                    </label>
                @endforeach
            </div>
        ...
        <x-slot name="footer">
            ...
            <x-buttons.submit class="ml-3" wire:click="updateApiToken" wire:loading.attr="disabled">
                {{ __('Save') }}
            </x-button>
        </x-slot>
    </x-dialog-modal>
